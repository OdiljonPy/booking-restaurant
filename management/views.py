...
class ManagementViewSet(viewsets.ViewSet):
    def create_manager(self, request, *args, **kwargs):
        ...
        if manager_serializer.is_valid():
            ...
            return Response({"message": "Manager Added Sucessfully", "status": status.HTTP_201_CREATED})
        return Response({"message": "please fill the datails", "status": status.HTTP_400_BAD_REQUEST})

    def show_managers(self, request):
        queryset = Manager.objects.all()
        managers = ManagerSerializer(queryset, many=True).data
        return Response(data={'managers': managers}, status=status.HTTP_200_OK)

class EmployerViewset(viewsets.ViewSet):
    def create_employer(self, request, *args, **kwargs):
        employer_serializer = EmployeeSerializer(data=request.data)
        if employer_serializer.is_valid():
            employer_serializer.save()
            return Response({"message": "Employee Added Sucessfully", "status": status.HTTP_201_CREATED})
        return Response({"message": "please fill the datails", "status": status.HTTP_400_BAD_REQUEST})

    def show_employer(self, request):
        queryset = Employee.objects.all()
        employers = EmployeeSerializer(queryset, many=True).data
        return Response(data={'employers': employers}, status=status.HTTP_200_OK)